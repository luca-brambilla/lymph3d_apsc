...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      ...
      do ie = 1, PolyMesh%num_elem_loc
         ...
         do i = 1, PolyMesh%Elem_loc(ie)%num_faces                   
           write(unit_mpi,*) 'Faces #: ',    PolyMesh%Elem_loc(ie)%faces(i,:)           
