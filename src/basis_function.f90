...
subroutine quadrature(nod2, wei2, nod3, wei3, p, nq3, nq2)
    ...
    ! Degree of exactness 3
        nod1 = [dsqrt(1.0d0/3.0d0), - dsqrt(1.0d0/3.0d0)]
        wei1 = [1.0d0, 1.0d0]
    end if
    if (nqn == 3) then
    ! Degree of exactness 5
        nod1 = [0.0d0, dsqrt(3.0d0/5.0d0), - dsqrt(3.0d0/5.0d0)]
        wei1 = [8.0d0/9.0d0, 5.0d0/9.0d0, 5.0d0/9.0d0]
    endif
    if (nqn == 4) then
    ! Degree of exactness 7
        nod1 = [0.3399810435848563d0, -0.3399810435848563d0, 0.8611363115940526d0, -0.8611363115940526d0]
        wei1 = [(18+dsqrt(30.0d0))/36.0d0, (18+dsqrt(30.0d0))/36.0d0, (18-dsqrt(30.0d0))/36.0d0, (18-dsqrt(30.0d0))/36.0d0]
    end if
    if (nqn == 5) then
    ! Degree of exactness 9
        nod1 = [0.0d0, 0.5384693101056831d0, -0.5384693101056831d0, 0.9061798459386640d0, -0.9061798459386640d0]
        wei1 = [128.0d0/225.0d0, (322+13*dsqrt(70.0d0))/900.0d0, (322+13*dsqrt(70.0d0))/900.0d0, (322-13*dsqrt(70.0d0))/900.0d0, (322-13*dsqrt(70.0d0))/900.0d0]
    end if
    if (nqn == 6) then
    ! Degree of exactness 11
        nod1 = [0.6612093864662645d0, -0.6612093864662645d0, 0.2386191860831969d0, -0.2386191860831969d0, 0.9324695142031521d0, -0.9324695142031521d0]
        wei1 = [0.3607615730481386d0, 0.3607615730481386d0, 0.4679139345726910d0, 0.4679139345726910d0, 0.1713244923791704d0, 0.1713244923791704d0]
    end if
    if (nqn == 7) then
    ! Degree of exactness 13
        nod1 = [0.0d0, 0.4058451513773972d0, -0.4058451513773972d0, 0.7415311855993945d0, -0.7415311855993945d0, 0.9491079123427585d0, -0.9491079123427585d0]
        wei1 = [0.4179591836734694d0, 0.3818300505051189d0, 0.3818300505051189d0, 0.2797053914892766d0, 0.2797053914892766d0, 0.1294849661688697d0, 0.1294849661688697d0]
    end if
...
subroutine GauLeg(a, b, n, x_GL, w_GL)
    ...
    do j=1,m
        z(j) = dcos( pi * ((j-0.25d0) / (n+0.5d0) ))
    enddo

    ! Error initialization in scalar and vectorial form
    Err = epsilon(1.0d0) + 1.0d0
    ErrV = Err * 1.0d0
    ...
    do while (Err > epsilon(1.0d0))
        ...
        do j = 1, n
            p(:,2:3) = p(:,1:2)
            p(:, 1) = ((2.0d0 * j - 1.0d0) / j) * z * p(:, 2) - ((j - 1.0d0) / j) * p(:, 3)
        end do

        ! Update of the new step solution
        p_app = n * (z * p(:, 1) - p(:, 2)) / (z**2 - 1.0d0)
        z_old = z
    ...
    if (MOD(n, 2) == 0) then
        x_GL = ((b + a) / 2.0d0) - ((b - a) / 2.0d0) * (/ z(1:(m-1)), -flip_z(2:m) /)
    else
        x_GL = ((b + a) / 2.0d0) - ((b - a) / 2.0d0) * (/ z(1:(m-1)), -flip_z /)
    end if
...
subroutine LegendreP_nonnorm(LP_nonnorm, x, p, int, nq)
    ...
    hb = (int(2) - int(1)) / 2.0d0
    mb = (int(2) + int(1)) / 2.0d0
    ...
    if (p > 7) then
        ...
        start = 7
        do ii = start, p - 1
            LP_temp = 1.0d0 / (ii + 1.0d0) * ((2*ii + 1)* xp*LP_old - ii*LP_oold)
            LP_oold = LP_old
            LP_old = LP_temp
        end do
...
subroutine basis(phi, dphi, b_box, Np, blist, Fk, nodtet3, nq3)
    ...
    do j=1,Np
        do q=1,nq3
            phi(j,q)=0.0d0 ! j-th basis function evaluated in the q-th 3D quadrature node
            do l=1,3
                dphi(l,j,q) = 0.0d0 ! partial derivative, w.r.t. the l-th coordinate, of the j-th basis function evaluated in the q-th 3D quadrature node
            end do
        end do
    end do
...
subroutine basis_boundary(phi_b, grad_b, e_E1, E2, b_box1, b_box2, blist, Np, Fk, node_maps, nodtria2, nq2)
    ...
    do j=1,Np
        do q=1,nq2
            phi_b(j,q,1)=0.0d0 ! j-th basis function, restricted in E1, evaluated in the q-th 2D quadrature node
            phi_b(j,q,2)=0.0d0
            do l=1,3
                grad_b(l,j,q,1) = 0.0d0 ! partial derivative w.r.t. the l-th component of the j-th basis function, restricted in E1, evaluated in the q-th 2D quadrature node
                grad_b(l,j,q,2) = 0.0d0
            end do
        end do
    end do
