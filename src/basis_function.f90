...
subroutine quadrature(nod2, wei2, nod3, wei3, p, nq3, nq2)
    ...
    ! Degree of exactness 7
        nod1 = [0.3399810435848563d0, -0.3399810435848563d0, &
        0.8611363115940526d0, -0.8611363115940526d0]
        wei1 = [(18+dsqrt(30.0d0))/36.0d0, (18+dsqrt(30.0d0))/36.0d0, &
        (18-dsqrt(30.0d0))/36.0d0, (18-dsqrt(30.0d0))/36.0d0]
    end if
    if (nqn == 5) then
    ! Degree of exactness 9
        nod1 = [0.0d0, 0.5384693101056831d0, -0.5384693101056831d0, &
        0.9061798459386640d0, -0.9061798459386640d0]
        wei1 = [128.0d0/225.0d0, (322+13*dsqrt(70.0d0))/900.0d0, &
        (322+13*dsqrt(70.0d0))/900.0d0, (322-13*dsqrt(70.0d0))/900.0d0, &
        (322-13*dsqrt(70.0d0))/900.0d0]
    end if
    if (nqn == 6) then
    ! Degree of exactness 11
        nod1 = [0.6612093864662645d0, -0.6612093864662645d0, &
        0.2386191860831969d0, -0.2386191860831969d0, 0.9324695142031521d0, &
        -0.9324695142031521d0]
        wei1 = [0.3607615730481386d0, 0.3607615730481386d0, &
        0.4679139345726910d0, 0.4679139345726910d0, 0.1713244923791704d0, &
        0.1713244923791704d0]
    end if
    if (nqn == 7) then
    ! Degree of exactness 13
        nod1 = [0.0d0, 0.4058451513773972d0, -0.4058451513773972d0, &
        0.7415311855993945d0, -0.7415311855993945d0, 0.9491079123427585d0, &
        -0.9491079123427585d0]
        wei1 = [0.4179591836734694d0, 0.3818300505051189d0, &
        0.3818300505051189d0, 0.2797053914892766d0, 0.2797053914892766d0, &
        0.1294849661688697d0, 0.1294849661688697d0]
    end if
...
subroutine LegendreP_nonnorm(LP_nonnorm, x, p, int, nq)
    ...
    if (p > 7) then
        ...
        start = 7
        do ii = start, p - 1
            LP_temp = 1.0d0 / (ii + 1.0d0) * &
            ((2*ii + 1)* xp*LP_old - ii*LP_oold)
            LP_oold = LP_old
            LP_old = LP_temp
        end do
